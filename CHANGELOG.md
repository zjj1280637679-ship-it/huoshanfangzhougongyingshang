...
- 发布新的插件视觉标识：`logo.png` 更换为“模型能力 4 项 → 5 项”的对比图标，突出补齐 `video / 视频` 后的原生多模态能力；使用新的版本号触发插件市场重新抓取版本与图标资源。
- 插件卡说明进一步明确 0.1.9 已完成的 QQ Silk/AMR 语音修复：不再仅相信临时文件扩展名，而是按真实音频内容识别、必要时解码，并统一归一化为 16 kHz、单声道、16-bit PCM WAV 后交给当前火山方舟主模型。
- 短简介同步强调“原生视频理解 + QQ 语音格式误判修复”，便于曾遇到 `input_audio is not of valid wav format` 的用户直接识别当前版本状态。
- 本版本不改变 Provider 核心请求逻辑、视频协议、音频归一化实现、普通 API / Agent Plan 固定端点或计费隔离行为；功能基线继续沿用已验证的 0.1.12 实现。
